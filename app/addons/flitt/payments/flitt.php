...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    if ($response === true) {
        ...
        if ($mode == 'ok' && $body['order_status'] == 'approved') {
            ...
            $pp_response = [
                'order_status' => 'F',
                'order_status_flitt' => $body['order_status'],
            ];
        }
    ...
    $payment_data = array(
        ...
        'currency' => $currency_f,
        'response_url' => fn_url("payment_notification.response?payment=flitt&order_id=" . $order_info['order_id'], AREA, 'current'),
    ...
    if ($response['result'] == true) {
        fn_create_payment_form($response['url'], [], 'Flitt', true, 'GET');
    } else {
